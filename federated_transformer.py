...
LOCAL_EPOCHS = 70        # Local epochs per round per client
BATCH_SIZE = 32
LR = 0.001
WEIGHT_DECAY = 1e-5
HIDDEN_DIM = 256
...
NUM_LAYERS = 2
DROPOUT = 0.1
THRESHOLD = 0.4         # Sigmoid threshold for binary classification
SEED = 42
...
class TransformerClassifier(nn.Module):
    def __init__(self, input_dim, num_heads=8, num_layers=2, hidden_dim=256, dropout=0.1):
        ...
        self.relu = nn.ReLU()
        self.fc2 = nn.Linear(hidden_dim // 2, 1)  # Single output for sigmoid
...
def evaluate(model, data_loader, device):
    ...
    with torch.no_grad():
        for X_batch, y_batch in data_loader:
            ...
            # Apply sigmoid and threshold 0.4
...
def run_federated(setting_label, X_train, X_test, y_train, y_test,
    ...
    """
    Runs one full federated training experiment.
...
os.makedirs(f'results/transformers/{NUM_CLIENTS}/round{NUM_ROUNDS}/epochs_{LOCAL_EPOCHS}/threshold_{THRESHOLD}', exist_ok=True)
...
#   alpha=0.5 -> moderately heterogeneous (current: 2.9% vs 0.3%)
#   alpha=1.0 -> somewhat more uniform
#   alpha=100 -> near-IID (almost homogeneous)
...
for exp in experiments:
    result = run_federated(
        ...
        mode=exp['mode'],
        alpha=exp['alpha'] if exp['alpha'] is not None else 0.5  # alpha unused for homogeneous
    )
...
summary_df.to_csv(f'results/transformers/{NUM_CLIENTS}/round{NUM_ROUNDS}/epochs_{LOCAL_EPOCHS}/threshold_{THRESHOLD}/federated_transformer_summary.csv', index=False)
print(f"\nSummary saved to 'results/transformers/{NUM_CLIENTS}/round{NUM_ROUNDS}/epochs_{LOCAL_EPOCHS}/threshold_{THRESHOLD}/federated_transformer_summary.csv'")
...
fig.suptitle(f'Federated Training — Accuracy per Round (N={NUM_CLIENTS} clients)', fontsize=14)

colors = ['tab:red', 'tab:orange', 'tab:green']
for ax, result, color in zip(axes, all_results, colors):
    rounds = list(range(1, len(result['round_accuracies']) + 1))
    ax.plot(rounds, result['round_accuracies'], marker='o', color=color, label='Accuracy')
...
plt.tight_layout()
plt.savefig(f'results/transformers/{NUM_CLIENTS}/round{NUM_ROUNDS}/epochs_{LOCAL_EPOCHS}/threshold_{THRESHOLD}/federated_accuracy_per_round.png', dpi=300)
print(f"Plot saved to 'results/transformers/{NUM_CLIENTS}/round{NUM_ROUNDS}/epochs_{LOCAL_EPOCHS}/threshold_{THRESHOLD}/federated_accuracy_per_round.png'")
...
fig.suptitle('Final Metrics Comparison Across Federated Settings', fontsize=14)
...
for ax, metric in zip(axes, metrics_to_plot):
    ...
    bars = ax.bar(x, values, color=['tab:red', 'tab:orange', 'tab:green'], edgecolor='black', width=0.5)
...
plt.tight_layout()
plt.savefig(f'results/transformers/{NUM_CLIENTS}/round{NUM_ROUNDS}/epochs_{LOCAL_EPOCHS}/threshold_{THRESHOLD}/federated_metrics_comparison.png', dpi=300)
print(f"Comparison plot saved to 'results/transformers/{NUM_CLIENTS}/round{NUM_ROUNDS}/epochs_{LOCAL_EPOCHS}/threshold_{THRESHOLD}/federated_metrics_comparison.png'")
...
fig.suptitle('Client Class Distribution per Federated Setting', fontsize=14)
...
plt.tight_layout()
plt.savefig(f'results/transformers/{NUM_CLIENTS}/round{NUM_ROUNDS}/epochs_{LOCAL_EPOCHS}/threshold_{THRESHOLD}/client_distribution_heatmap.png', dpi=300)
print(f"Distribution heatmap saved to 'results/transformers/{NUM_CLIENTS}/round{NUM_ROUNDS}/epochs_{LOCAL_EPOCHS}/threshold_{THRESHOLD}/client_distribution_heatmap.png'")
